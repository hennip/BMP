...
# Perusmalli, jokainen pullosarja mallinnetaan toisistaan riippumattomina
 # Kokeessa 1 ei ole nähdäkseni pointtia ns lainata muilta sarjoilta cv:n oppimisessa. 
 # Tulokset selkeät, mikään näytteistä ei tuota kaasua paremmin kuin pelkkä biojäte
  ...
  for(i in 1:n_days_y[3]){ # number of days per experiment 
  ...
  for(i in 1:n_days_x[1]){ # number of days
    for(j in 1:3){ # number of bottles
      ...
      x5[i,j]~dlnorm(Mx[i,5],Tx[i,5]) 
      x6[i,j]~dlnorm(Mx[i,6],Tx[i,6]) 
      x7[i,j]~dlnorm(Mx[i,7],Tx[i,7]) 
      x8[i,j]~dlnorm(Mx[i,8],Tx[i,8]) 
      x9[i,j]~dlnorm(Mx[i,9],Tx[i,9]) 
      #x10[i,j]~dlnorm(Mx[i,10],Tx[i,10]) 
    }
  }
  for(k in 1:n_types[1]){# number of sample types
    for(i in 1:n_days_x[1]){ # number of days
      ...
      erotus[i,k]<-cum_x_new[i,k]-cum_y_new1[i]
      tulos[i,k]<-erotus[i,k]/const_x[k]
    }
    cum_x_new[1,k]<-x_new[1,k]
    for(i in 2:n_days_x[1]){
...
model{
 ...
 # Tulokset selkeät, mikään näytteistä ei tuota kaasua paremmin kuin pelkkä biojäte
  ...
  for(i in 1:n_days[1]){ # number of days per experiment 
    for(j in 1:nb_y[1]){ # number of bottles per experiment
      y1[i,j]~dlnorm(My1[i],Ty1[i]) # y: inoculum
    }
    My1[i]<-log(muy1[i])-0.5/Ty1[i]
  ...
  for(i in 1:n_days[2]){ # number of days per experiment 
    for(j in 1:nb_y[2]){ # number of bottles per experiment
      y2[i,j]~dlnorm(My2[i],Ty2[i]) # y: inoculum
    }
    My2[i]<-log(muy2[i])-0.5/Ty2[i]
  ...
  for(i in 1:n_days[1]){ # number of days
    for(j in 1:3){ # number of bottles
      x1[i,j]~dlnorm(Mx[i,1],Tx[i,1]) 
      x2[i,j]~dlnorm(Mx[i,2],Tx[i,2]) 
      x3[i,j]~dlnorm(Mx[i,3],Tx[i,3]) 
...
data<-list(
  ...
  nb_y=nb_y,
  n_types=n_types,
   #const_x=rep(25.46,n_types[1]),
  #const_x=rep(1,n_types[1]),
  const_x=rep(12.73,n_types[1]),
  x1=df_x1[[1]],x2=df_x1[[2]],x3=df_x1[[3]],
   x4=df_x1[[4]],x5=df_x1[[5]],x6=df_x1[[6]],
   x7=df_x1[[7]],x8=df_x1[[8]],
   x9=df_x1[[9]],#x10=df_x[[10]],
...
run1 <- run.jags(M1, 
                 ...
                 progress.bar=TRUE)
run2<-extend.jags(run1, sample = 2000, thin=20, keep.jags.files = F, combine = T)
...
summary(run, var="eta")
summary(run, var="psi")
summary(run, var="cv_x_pred")
summary(run, var="cv_y_pred")
