...
# Perusmalli, jokainen pullosarja mallinnetaan toisistaan riippumattomina
  ...
  for(k in 1:n_types[3]){# number of sample types
    for(i in 1:n_days_x){ # number of days
...
M2<-"
model{
# Kokeessa 3 ymppipulloja vain kaksi -> epävarmuus liioitellun suurta.
# => mallinnetaan ymppipullot yhteisten hierarkkisten priorien avulla 
# kolmen kokeen yli jotta cv saadaan järkeväksi
  ...
  for(k in 1:n_types[3]){# number of sample types
    ...
    tulos[k]<-(cum_x_new[n_days_x,k]-cum_y_new3[n_days_y[3]])/const_x[k]
...
Mname<-"eta&psicommonfory_koe3_const2"
...
data<-list(
  ...
  nb_y=nb_y,
  n_types=n_types,
   #const_x=rep(8.04,n_types[3]),
  const_x=rep(2.71,n_types[3]),
  x1=df_x3[[1]],x2=df_x3[[2]],x3=df_x3[[3]],
   x4=df_x3[[4]],x5=df_x3[[5]],x6=df_x3[[6]],
   x7=df_x3[[7]],x8=df_x3[[8]],
   x9=df_x3[[9]],x10=df_x3[[10]],
...
save(run, file=str_c(pathMain,"01-Projects/BMP/output/run2_koe3.RData"))
