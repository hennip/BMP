...
model{
  ...
  for(i in 1:n_days){ # number of days
    for(j in 1:2){ # number of bottles
  ...
  for(i in 1:n_days){ # number of days
    for(j in 1:3){ # number of bottles
      ...
      x9[i,j]~dlnorm(Mx[i,9],Tx[i,9]) # x9: NÄYTE pyro fuugattu kuusi
      x10[i,j]~dlnorm(Mx[i,10],Tx[i,10]) # x10: NÄYTE pyro fuugattu kuusi
    }
  }
...
save(run, file=str_c(pathMain,"01-Projects/BMP/output/run1_koe3.RData"))
